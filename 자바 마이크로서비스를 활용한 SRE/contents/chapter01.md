...
## 캡슐화
**캡슐화** → 핵심 개념은 객체의 상태를 외부에서 알 수 없게 감추는 것이며 **정보 은닉**으로 표현한다.
- 플랫폼 엔지니어링팀의 업무도 캡슐화와 비슷한 면이 많다.
- 고객은 개발팀이며 우수한 복원 관행을 확립해 고객에게 제공한다.
- 통제에서 벗어나기 위해서가 아니라 고객의 책임을 덜어주기 위해 정보를 은닉한다.

**명시적 런타임 종속성**
- 모든 마이크로서비스에 런타임 종속성으로 포함된 라이브러리가 있다면 거의 확고한 전달 메커니즘으로 볼 수 있다.
- 주요 메트릭으로 삼고, 텔레메트리 태그를 심고, 트래픽 관리 패턴을 추가한다.
- 스프링 프레임워크 사용 비중이 높다면 auto configuration 클래스를 활용한다.

**서비스 클라이언트 종속성**
- 트래픽 관리 패턴(fallback, retry 등) 측면에서 보면, 서비스 개발팀이 서비스 클라이언트도 함께 담당하는 것도 고려할 만하다.
- 서비스 클라이언트는 서비스와 상호작용한다. 서비스의 약점과 장애 위험 요인을 가장 잘 아는 부서는 결국 서비스를 개발하고 운영하는 팀이다.
- 서비스 엔지니어야말로 이러한 지식을 클라이언트 의존성에 구현할 수 있는 최적임자이다. → 소비자는 가장 믿을 수 있는 경로로 서비스를 이용할 수 있다.

**런타임 종속성 주입**
- 배포 절차가 비교적 표준화된 상황이라면 배포 환경에 런타임 종속성을 주입할 수 있다.
- Cloud Foundry 위에서 실행되는 스프링부트 애플리케이션은 배포 환경에서 플랫폼 메트릭 구현을 주입한다.

### 서비스 메시
애플리케이션의 사이드카 프로세스 또는 컨테이너와 관련된 플랫폼 공통 기능을 캡슐화한다. Service Mesh는 control plane 관리와 사이드카 프로세스를 한데 묶어 지칭하는 용어다.

- 애플리케이션 외부에서 마이크로서비스 사이의 상호작용을 관리하는 인프라 계층이다.
- Istio
- 사이드카 플랫폼은 애플리케이션을 대신해 트래픽 관리, 서비스 탐색, 모니터링 등을 담당한다.
- 애플리케이션을 더 단순하게 개발할 수 있으며 서비스 배포 및 운영 과정에서 증가하는 복잡도와 비용을 상쇄하는 효과가 있다.
- Istio가 추구하는 개념은 중앙 제어 플레인을 통해 마이크로서비스의 전체적인 정책을 관리하고 전파하는 것이다.
  - 넷플릭스 OSS 스택의 강점을 최대한 캡슐화했으며 아직 실현하지 못한 잠재적 가능성은 다음 탈중앙화 주기에서 해소된다.
  - Resilience4j → 적응형 벌크헤드 패턴

![Image](https://github.com/user-attachments/assets/90755612-e271-4133-95d2-a72d7791079e)

도메인별 지식이 부족하면 사이드카의 규모도 정하기 어렵다. 애플리케이션이 1분 동안 처리해야 할 요청이 10,000건인지 1건인지 알 수 없다.